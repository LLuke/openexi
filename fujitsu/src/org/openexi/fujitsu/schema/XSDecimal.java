...
public final class XSDecimal {
   ...
   */
  public static XSDecimal parse(String norm)
    ...
    int pos, len, mode;
    for (pos = 0, len = norm.length(), mode = DECIMAL_MODE_MAYBE_SIGN;
         pos < len && !syntaxInvalid; pos++) {
    ...
    if (!syntaxInvalid) {
       ...
       *   // Assume there is at least one fraction digits when decimal point
    ...
    if (syntaxInvalid ||
        mode != DECIMAL_MODE_IS_INTEGRAL && mode != DECIMAL_MODE_IS_FRACTION)
      throw new SchemaValidatorException(
          SchemaValidatorException.INVALID_DECIMAL,
          new String[] { norm }, (EXISchema)null, EXISchema.NIL_NODE);
    else if (digits == 0)
      throw new SchemaValidatorException(
          SchemaValidatorException.INVALID_DECIMAL,
          new String[] { norm }, (EXISchema)null, EXISchema.NIL_NODE);
  ...
  public boolean equals(Object obj) {
    if (obj != null && obj instanceof XSDecimal) {
      if ((m_value.compareTo(((XSDecimal)obj).m_value) == 0))
        return true;
    }
    return false;
  }
