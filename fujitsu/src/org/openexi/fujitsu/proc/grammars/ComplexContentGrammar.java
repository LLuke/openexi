...
final class ComplexContentGrammar extends ElementContentGrammar {
  ...
  ComplexContentGrammar(int tp, GrammarCache cache, boolean checkSanity) {
    ...
    assert m_nodes[m_initials[POS_2]] == EXISchema.NIL_NODE;
    ...
    for (i = 0, len = m_initialParticles.length; i < len; i++) {
      m_initialParticles[i] = m_nodes[m_initials[m_initialCursor] + sortedInitials[m_initialCursor][i]];
    }
    ...
    for (i = 0; i < POS_SZ; i++) {
      ...
      for (j = 0, serial = 0; j < len; j++, serial += eventTypes.length) {
        eventTypes = createEventType(m_nodes[m_initials[i] + sortedInitials[i][j]], sortedInitials[i][j], serial, this, m_eventTypeLists[i]);
  ...
  private void updateState(GrammarState stateVariables) {
    ...
    stateVariables.phase = m_nodes[initials + n_initials - 1] != EXISchema.NIL_NODE ? ELEMENT_STATE_CONTENT_DEPLETE :
