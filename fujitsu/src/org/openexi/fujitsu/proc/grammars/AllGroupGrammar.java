...
final class AllGroupGrammar extends GroupGrammar {
  ...
  AllGroupGrammar(int group, Grammar parent, GrammarCache cache) { 
    ...
    m_initials = m_schema.getHeadSubstanceListOfGroup(m_nd);
    m_indices = m_initials + n_initials;
    if (m_nodes[m_initials + n_initials - 1] == EXISchema.NIL_NODE)
      --n_initials;
    m_n_initials = n_initials;
    
    assert m_n_particles == m_n_initials;
    ...
    int serial;
    for (i = 0, serial = 0; i < m_n_initials; i++) {
      final int particle;
      if ((particle = m_nodes[m_initials + i]) != EXISchema.NIL_NODE) {
        eventTypes = createEventType(particle, m_nodes[m_indices + i], serial, this, m_eventTypeList);
  ...
  void element(int eventTypeIndex, String uri, String name, GrammarState stateVariables) {
    ...
    final int ind = m_nodes[m_indices + matchedPos];
