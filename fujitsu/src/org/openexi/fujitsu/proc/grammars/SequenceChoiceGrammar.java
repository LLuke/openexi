  ...
  SequenceChoiceGrammar(int group, Grammar parent, GrammarCache cache,
    ...
    for (i = 1, triplet = m_n_triplets - 1; triplet >= 0; triplet--, i++) {
      ...
      for (int j = 0; j < length; j++) {
        final int particle = m_nodes[offset + j];
        if (particle != EXISchema.NIL_NODE || triplet == 0) {
          final int index = m_nodes[indices + j];
    ...
    for (i = 0; i <= m_n_particles; i++) {
      ...
      byte acceptance;
      if (m_nodes[group_initials[2 * i] + (group_n_initials[2 * i] - 1)] == EXISchema.NIL_NODE) {
      ...
      if (m_nodes[group_initials[2 * i + 1] + (group_n_initials[2 * i + 1] - 1)] == EXISchema.NIL_NODE) {
      ...
      for (pos = 0, j = 0, len = group_n_initials[2 * i + 1]; j < len; j++) {
        final int particle = m_nodes[group_initials[2 * i + 1] + j];
        assert particle != EXISchema.NIL_NODE || j == len - 1;
        if (particle != EXISchema.NIL_NODE) {
          m_initials[2 * i + 1][pos] = particle;
          m_indices[2 * i + 1][pos] = m_nodes[group_indices[2 * i + 1] + j];
    ...
    for (i = 0; i < m_eventTypes.length; i++) {
      ...
      for (j = 0, serial = 0; j < len; j++) {
        final int substanceParticle1 = m_nodes[group_initials[i] + group_sortedInitials[i][j]];
        if (groupAcceptance != GROUP_STATE_NOT_DONE) {
          ...
          while (serial < eventTypeList.size()) {
            ...
            if (eventType.itemType == EventCode.ITEM_SCHEMA_EE) {
              ...
              if (sn1 == sn2) {
                int particle1 = m_particles[m_nodes[group_indices[i] + group_sortedInitials[i][j]]];
          ...
          eventTypeSchemas = createEventType(m_nodes[group_initials[i] + group_sortedInitials[i][j]], 
