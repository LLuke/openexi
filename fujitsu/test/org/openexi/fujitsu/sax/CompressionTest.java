...
import org.openexi.fujitsu.proc.events.EXIEventAttributeByRef;
import org.openexi.fujitsu.proc.events.EXIEventWildcardAttributeByRef;
...
public class CompressionTest extends TestBase {
  ...
  private static final AlignmentType[] Alignments = new AlignmentType[] { 
    AlignmentType.bitPacked, AlignmentType.byteAligned,
  ...
  public void testXsiNil_01() throws Exception {
    ...
    for (AlignmentType alignment : alignments) {
      for (boolean preserveWhitespaces : new boolean[] { true, false }) {
        ...
        Assert.assertTrue(exiEvent instanceof EXIEventWildcardAttributeByRef); // was in value channel
        ...
        Assert.assertEquals("http://www.w3.org/2001/XMLSchema-instance", exiEvent.getURI());
        Assert.assertEquals("true", exiEvent.getCharacters().makeString());
        Assert.assertTrue(exiEvent instanceof EXIEventAttributeByRef); // was in value channel
