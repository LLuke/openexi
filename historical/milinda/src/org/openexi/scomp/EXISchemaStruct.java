...
abstract class EXISchemaStruct {
  ...
  protected int[]        m_opaques;      // int array containing opaque nodes
  ...
  protected int      m_n_opaques;   // number of ints used in m_opaques
  ...
  protected int m_typesCount; // used for numbering types, counting the number of types
  ...
  private static final int OPAQUES_INITIAL   = 1024;
  ...
  protected void reset() {
    ...
    m_opaques      = new int[OPAQUES_INITIAL];
    ...
    m_n_elems = 0;
    m_n_attrs = 0;
    m_typesCount = 0; // 0 represents anyType.
    m_n_stypes = 0;
    m_n_groups = 0;

    m_uris[m_n_uris++] = "";
    m_uris[m_n_uris++] = "http://www.w3.org/XML/1998/namespace";
    m_uris[m_n_uris++] = "http://www.w3.org/2001/XMLSchema-instance";
    m_uris[m_n_uris++] = "http://www.w3.org/2001/XMLSchema";
    
    int i;
    m_names[m_n_names++] = "";
    for (i = 0; i < StringTableConst.XML_LOCALNAMES.length; i++)
      m_names[m_n_names++] = StringTableConst.XML_LOCALNAMES[i];
    for (i = 0; i < StringTableConst.XSI_LOCALNAMES.length; i++)
      m_names[m_n_names++] = StringTableConst.XSI_LOCALNAMES[i];
    for (i = 0; i < StringTableConst.XSD_LOCALNAMES.length; i++)
      m_names[m_n_names++] = StringTableConst.XSD_LOCALNAMES[i];
  ...
  private final void ensureOpaques(int size) {
    while (m_n_opaques + size > m_opaques.length) {
      int[] opaques = new int[2 * m_opaques.length];
      System.arraycopy(m_opaques, 0, opaques, 0, m_opaques.length);
      m_opaques = opaques;
    }
  }
   ...
   * space allocation, which means m_n_opaques is added up in this method.
  ...
  protected final int allocOpaque(int size) {
    ...
    ensureOpaques(nodeSize);
    final int nd = m_n_opaques;
    m_opaques[nd] = EXISchema.OPAQUE_NODE;
    m_opaques[nd + 1] = nodeSize;
    m_n_opaques += nodeSize;
  ...
  protected final int internUri(String uri) {
    ...
    if (uri.equals(XMLSCHEMA_INSTANCE_ALTURI))
      uri = XMLSCHEMA_INSTANCE_URI;
