...
public abstract class Scanner extends Apparatus {
  ...
  public final EXIEventSchemaNil readXsiNilValue(String prefix, EventType eventType) throws IOException {
    if (m_preserveLexicalValues) {
      ...
      characterSequence = m_valueScannerTable[CODEC_STRING].scan("nil", XmlUriConst.W3C_2001_XMLSCHEMA_INSTANCE_URI, 
  ...
  protected final EXIEventSchemaType readXsiTypeValue(String prefix, EventType eventType) throws IOException {
    ...
    if (m_preserveLexicalValues) {
      characterSequence = m_valueScannerTable[CODEC_STRING].scan("type", XmlUriConst.W3C_2001_XMLSCHEMA_INSTANCE_URI, 
          EXISchema.NIL_NODE, m_inputStream);
      final int i;
  ...
  protected final CharacterSequence readLiteralString(int ucsCount, int tp, InputStream istream) throws IOException {
    ...
    final int[] rcs;  
    if (tp > 0) {
      final int ancestryId = m_schema.getAncestryIdOfSimpleType(tp);
      n_chars = ancestryId == EXISchemaConst.STRING_TYPE ? m_schema.getRestrictedCharacterCountOfSimpleType(tp) : 0;
      if (n_chars != 0) {
  ...
  public final class BooleanValueScanner extends ValueScannerBase {
    ...
    public CharacterSequence scan(String localName, String uri, int tp, InputStream istream) throws IOException {
      if (istream == null)
        istream = m_istream;
      if (m_schema.isPatternedBooleanSimpleType(tp)) {
  ...
  public final class EnumerationValueScanner extends ValueScannerBase {
    ...
    public CharacterSequence scan(String localName, String uri, int tp, InputStream istream) throws IOException {
      ...
      if (m_schema.getVarietyOfSimpleType(tp) == EXISchema.LIST_SIMPLE_TYPE) {
        String stringValue = "";
        assert m_schema.getTypeOfVariant(facet) == EXISchema.VARIANT_LIST;
        int[] variants = m_schema.getListValueOfVariant(facet);
        ...
        for (int i = 0; i < n_variants; i++) {
          if (i != 0)
            stringValue += " ";
          stringValue += getStringValueOfFacet(itemType, itemAncestryId, variants[i]);
        }
        ...
        return getCharacterSequenceOfFacet(tp, facet);
      }
    }
  }

  private CharacterSequence getCharacterSequenceOfFacet(int tp, int facet) {
    final int ancestryId = m_schema.getAncestryIdOfSimpleType(tp);
    if (ancestryId == EXISchemaConst.BASE64BINARY_TYPE) {
      ...
      final String stringValue = getStringValueOfFacet(tp, ancestryId, facet);
      final int length = stringValue.length();
      final CharacterBuffer characterBuffer = ensureCharacters(length);
      return characterBuffer.addString(stringValue, length);
    }
  ...
  private String getStringValueOfFacet(int tp, int ancestryId, int facet) {
    final byte[] binaryValue;
    String stringValue;
    switch (ancestryId) {
      ...
      case EXISchemaConst.BASE64BINARY_TYPE:
        ...
        final int n_chars = Base64.encode(binaryValue, binaryValue.length, characters, 0);
        return new String(characters, 0, n_chars);
  ...
  public final class Base64BinaryValueScanner extends ValueScannerBase {
    ...
    public Base64BinaryValueScanner() {
      super(new QName("exi:base64Binary", ExiUriConst.W3C_2009_EXI_URI));
  ...
  public final class HexBinaryValueScanner extends ValueScannerBase {
    ...
    public HexBinaryValueScanner() {
      super(new QName("exi:hexBinary", ExiUriConst.W3C_2009_EXI_URI));
      m_stringBuffer = new StringBuffer();
    }
    @Override
    public short getCodecID() {
