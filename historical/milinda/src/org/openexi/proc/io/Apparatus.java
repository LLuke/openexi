        ...
        switch (m_schema.getVarietyOfSimpleType(stype)) {
          ...
          case EXISchema.ATOMIC_SIMPLE_TYPE:
            ...
            if (_codecID == CODEC_NOT_A_CODEC) {
              do {
                tp = m_schema.getBaseTypeOfAtomicSimpleType(tp);
                if ((_codecID = m_codecTable[m_schema.getSerialOfType(tp)]) != CODEC_NOT_A_CODEC)
                  break;
              }
              while (true);
            }
            final int ancestryId = m_schema.getAncestryIdOfSimpleType(stype);
            if (ancestryId != EXISchemaConst.QNAME_TYPE && ancestryId != EXISchemaConst.NOTATION_TYPE) {
              if (m_schema.getEnumerationFacetCountOfSimpleType(stype) != 0) {
                if (stype == tp || m_schema.getEnumerationFacetCountOfSimpleType(tp) == 0)
