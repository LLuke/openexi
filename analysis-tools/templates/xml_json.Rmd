...
Use this chunk to set the parameters for a test run. `input.file` is the path to an input file in the above CSV format. `util.file` is the path to util.R, the plotting functions for graphing. Leave it alone unless you've changed the directory structure. _Beware OS path separators (`\` vs. `/`)_

```{r}
input.file = "../sample_inputs/sample-gpx-results.csv"
```
...
source("../utilities/filesize-vs-compaction.R")
