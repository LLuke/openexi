...
// Create UI components.    
    ...
    private Checkbox checkbox_useSerializedSchema = new Checkbox();
    private Checkbox checkbox_strict = new Checkbox();
    private Button button_serializeSchema = new Button();
    private TextField textField_esdFile = new TextField();
    private Label label_esdFileName = new Label();
    private TextField textField_xsdFile = new TextField();
    private Label label_xsdFileName = new Label();
    private Button button_browseSchema = new Button();
    private Checkbox checkbox_useXsd = new Checkbox();
    private Checkbox checkbox_noSchema = new Checkbox();
    private CheckboxGroup checkboxGroup_schema = new CheckboxGroup();
...
// The frame is large, but subsequent examples will require additional controls.
        ...
        checkbox_useSerializedSchema.setLabel("EXIG");
        checkbox_useSerializedSchema.setBounds(new Rectangle(275, 505, 65, 20));
        checkbox_useSerializedSchema.setCheckboxGroup(checkboxGroup_schema);
        checkbox_strict.setLabel("Strict");
        checkbox_strict.setBounds(new Rectangle(385, 505, 50, 20));
        checkbox_strict.addItemListener(new ItemListener() {
                public void itemStateChanged(ItemEvent e) {
                    checkbox_strict_itemStateChanged(e);
                }
            });
        button_serializeSchema.setLabel("Serialize EXIG");
        button_serializeSchema.setBounds(new Rectangle(345, 465, 95, 25));
        button_serializeSchema.setActionCommand("button_serializeSchema");
        button_serializeSchema.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    button_serializeSchema_actionPerformed(e);
                }
            });
        textField_esdFile.setBounds(new Rectangle(120, 465, 220, 25));
        textField_esdFile.setText("test.exig");
        textField_esdFile.setFont(new Font("Tahoma", 0, 14));
        label_esdFileName.setText("EXIG File:");
        label_esdFileName.setBounds(new Rectangle(20, 465, 100, 30));
        label_esdFileName.setFont(new Font("Tahoma", 0, 14));
        textField_xsdFile.setBounds(new Rectangle(120, 430, 220, 25));
        textField_xsdFile.setFont(new Font("Tahoma", 0, 14));
        textField_xsdFile.setText("test.xsd");
        label_xsdFileName.setText("XSD File:");
        label_xsdFileName.setBounds(new Rectangle(20, 430, 100, 25));
        label_xsdFileName.setFont(new Font("Tahoma", 0, 14));
        button_browseSchema.setLabel("Browse...");
        button_browseSchema.setBounds(new Rectangle(345, 430, 95, 25));
        button_browseSchema.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    button_browseSchema_actionPerformed(e);
                }
            });
        checkbox_useXsd.setLabel("XSD");
        checkbox_useXsd.setBounds(new Rectangle(200, 505, 65, 20));
        checkbox_useXsd.setCheckboxGroup(checkboxGroup_schema);
        checkbox_noSchema.setLabel("None");
        checkbox_noSchema.setBounds(new Rectangle(115, 505, 75, 20));
        checkbox_noSchema.setCheckboxGroup(checkboxGroup_schema);
        checkbox_noSchema.setState(true);
        label1.setText("Use Schema:");
        label1.setBounds(new Rectangle(20, 505, 90, 20));
        label1.setFont(new Font("Tahoma", 0, 14));
        this.getContentPane().add(label1, null);
        this.getContentPane().add(checkbox_noSchema, null);
        this.getContentPane().add(checkbox_useXsd, null);
        this.getContentPane().add(button_browseSchema, null);
        this.getContentPane().add(label_xsdFileName, null);
        this.getContentPane().add(textField_xsdFile, null);
        this.getContentPane().add(label_esdFileName, null);
        this.getContentPane().add(textField_esdFile, null);
        this.getContentPane().add(button_serializeSchema, null);
        this.getContentPane().add(checkbox_strict, null);
        this.getContentPane().add(checkbox_useSerializedSchema, null);
...
// This example adds arguments for compiling EXI documents using a schema.             
                textField_xsdFile.getText(),
                textField_esdFile.getText(),
                checkbox_strict.getState(),
// Possible values are "None", "XSD", "EXIG".
                checkboxGroup_schema.getSelectedCheckbox().getLabel()
        );
...
// Preservation options, with the exception of Preserve Lexical Values, 
// are not applicable when using the STRICT option. Disabling the checkboxes
// when Strict is selected is a handy reminder.
    
    private void checkbox_strict_itemStateChanged(ItemEvent e) {
        if (checkbox_strict.getState()) {
            label_results.setText("Using Strict schema option (custom settings ignored).");
        }
        else {
            label_results.setText("Using Default schema option.");            
        }
        checkbox_preserveWhitespace.setEnabled(!checkbox_strict.getState());
        checkbox_preserveComments.setEnabled(!checkbox_strict.getState());
        checkbox_preservePIs.setEnabled(!checkbox_strict.getState());
        checkbox_preserveDTD.setEnabled(!checkbox_strict.getState());
        checkbox_preserveNS.setEnabled(!checkbox_strict.getState());
    }

// Serialize an XSD to EXIG. The only arguments required are source and target files.
    private void button_serializeSchema_actionPerformed(ActionEvent e) {
        SerializeEXISchema ss = new SerializeEXISchema();
        try {
            ss.serializeEXISchema(
                textField_xsdFile.getText(),
                textField_esdFile.getText()
            );
            label_results.setText("Compiled schema to " 
            + textField_esdFile.getText() + ".");
        } catch (IOException f) {
            if (debug) f.printStackTrace();
        } catch (EXISchemaFactoryException sfe) {
            label_results.setText("Schema error: " + sfe.getMessage());
            sfe.printStackTrace();
        }
    }

// Use a file dialog to get the path and name of an XSD document.
    private void button_browseSchema_actionPerformed(ActionEvent e) {
        label_results.setText("");
        FileDialog fd = new FileDialog(this);
        String fileName;
        String extension;
        fd.setVisible(true);
        textField_xsdFile.setText(fd.getDirectory() + fd.getFile());

// For convenience, insert a suggested *.exig file name, based on the selected XSD file.
        textField_esdFile.setText(
            fd.getDirectory() 
            + fd.getFile().substring(0,fd.getFile().lastIndexOf(".")) 
            + ".exig"
        );
    }
}
