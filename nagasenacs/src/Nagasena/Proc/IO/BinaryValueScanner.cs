...
using IBinaryValueScanner = Nagasena.Proc.Common.IBinaryValueScanner;
using QName = Nagasena.Proc.Common.QName;

namespace Nagasena.Proc.IO {

  internal abstract class BinaryValueScanner : ValueScannerBase, IBinaryValueScanner {

    protected internal readonly Scanner m_scanner;

    // REVISIT: Can we abandon m_octets?
    protected internal byte[] m_octets;
    private readonly OctetBuffer m_octetBuffer;
    private readonly bool m_resetBeforeScan;

    public BinaryValueScanner(QName qname, Scanner scanner) : base(qname) {
      m_scanner = scanner;
      m_octets = new byte[8192];
      m_octetBuffer = m_scanner.octetBuffer;
      AlignmentType alignmentType = scanner.AlignmentType;
      m_resetBeforeScan = alignmentType == AlignmentType.bitPacked || alignmentType == AlignmentType.byteAligned;
    }

    protected internal void expandBuffer(int len) {
      int _length;
    ...
    public BinaryDataSource scan(long n_remainingBytes, BinaryDataSource binaryDataSource) {
      ...
      if (n_nextBytes == -1) { // i.e. unspecified n_nextBytes
        n_nextBytes = n_remainingBytes > int.MaxValue ? int.MaxValue : (int)n_remainingBytes;
      }
